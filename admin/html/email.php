...
<?php
if(isset($_GET['spam'])){
echo '<b style="color:blue;font-size:20px;">'.$_GET['spam'].'</b>';	
} else {
...
$message2 = trim(mysqli_real_escape_string($conn,$_POST['message']));
$subjects = trim(mysqli_real_escape_string($conn,$_POST['subjects']));

$to = $recipient;
$subject = $subjects;

$message  = "<p style='color:blue;text-align:justify;background:white;margin:5px;'>".$message2."</p>";
$message .= "<h1 style='color:white;background:rgba(10, 148, 219, 1);border-radius:5px 5px;'>".$school_name."<br/>
<i style='color:white;font-size:20px;'>POWERED BY SKOOLWEB</i></h1>";
...
VALUES('$subjects','$logo','$admin','$recipient','$message2',now(),'$school_name')";
...
$sql = "SELECT COUNT(id) FROM email WHERE schoolidentity = '$school_name' AND recipient = '$admin_email'";
$query = mysqli_query($conn,$sql);
...
$sql = "SELECT * FROM email WHERE schoolidentity = '$school_name' AND recipient = '$admin_email' ORDER BY id DESC";	
$query = mysqli_query($conn, $sql);
...
<a href="javascript:remove2(<?php echo $row['id'];?>)" class="btn-rounded btn btn-default btn-outline"><i class="fa fa-star"></i> mark as spam</a>
</div>
</div>
</div>
...
function remove2(id)
{
if(confirm('are you sure you want to mark message as spam?'))
{
window.location='spam.php?remove_id='+id;
}
}
</script>
<?php
...
$alert = "NO mail YET!!!!!";	
echo '<p style="color:blue;">'.$alert.'</p>';
}
?>
<div id="textline">
